...
for my $rl1_sequence (@rl1_sequenceResults) {
  ...
  if (exists($list_links->{$rl1_sequence->list_id})) {
    ...
    if (defined($rl1_material)) {
      ...
      my $name = defined($rl1_material->title) ? fix_latin(decode_entities($rl1_material->title)) : '';
...
my $roleResult = $rebus2->resultset('Role')->find({name => 'author'});
my $rl2_editorID = $roleResult->id;
...
$roleResult = $rebus2->resultset('Role')->find({name => 'leader'});
