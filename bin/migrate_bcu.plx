...
# Find next root
my $rootResult = $rebus2->resultset('List')->search({}, {order_by => {'-asc' => [qw/root_id/]}, rows => '1'})->single;
my $rootID;
...
# Add dupesResult
$rootID = $rootID - 1;
my $dupesResult = $rebus2->resultset('List')->create(
  {
...
$dupesResult->update({'source_uuid' => $config->{'code'} . "-" . $dupesResult->id});
$dupesResult->discard_changes;

# Add unmappedResult
$rootID = $rootID - 1;
my $unmappedResult = $rebus2->resultset('List')->create(
  {
    name                     => 'UNMAPPED LISTS',
    source_id                => 1,
    suppressed               => 1,
...
$unmappedResult->update({'source_uuid' => $config->{'code'} . "-" . $unmappedResult->id});
...
my $rl1_listResults = $rebus1->resultset('List')->search(undef, {order_by => {'-desc' => [qw/published_yn/]}});
for my $rl1_list ($rl1_listResults->all) {
  ...
  my $rl2_list;
  my $course_identifier = defined($rl1_list->course_identifier)
    ? uc(
    ...
    : '';
      ...
      $rl2_list = $unmappedResult->create_rightmost_child(
        {
          name                     => fix_latin(decode_entities($rl1_list->list_name)),
      ...
      if (exists $courses{$course_identifier}) {
        $rl2_list = $dupesResult->create_rightmost_child(
          {
            name                 => "[" . $course_identifier . "] " . fix_latin(decode_entities($rl1_list->list_name)),
            no_students          => $rl1_list->no_students,
            ratio_books          => $rl1_list->ratio_books,
            ratio_students       => $rl1_list->ratio_students,
            updated              => $dt,
...
# Update Sequence
  ...
  my @rl1_unitResults = $rebus1->resultset('OrgUnit')
    ->search({name => {'LIKE' => '% - %'}}, {order_by => {'-asc' => [qw/parent org_unit_id/]}})->all;

  for my $rl1_unitResult (@rl1_unitResults) {
    my $code = substr($rl1_unitResult->name, 0, index($rl1_unitResult->name, ' '));

    my $rl2_list = $rebus2->resultset('List')->search({course_identifier => $code});
    if ($rl2_list->count == 1) {
...
# User, UserType
  ...
  for my $rl1_user (@rl1_userResults) {
    ...
    my $email = defined($rl1_user->email_address) ? lc($rl1_user->email_address) : 'not@myemail.com';
    my $login = defined($rl1_user->login)         ? lc($rl1_user->login)         : 'login' . $current_line;

    # Add user
    my $usertype_name = defined($role_map->{$rl1_user->type_id}) ? $role_map->{$rl1_user->type_id} : 'public';
    my $rl2_user;
    $rl2_user = $rebus2->resultset('User')->find({login => $email}, {key => 'login'});
    $rl2_user = $rebus2->resultset('User')->find({login => $login}, {key => 'login'}) unless defined($rl2_user);
    $rl2_user = $rebus2->resultset('User')->create(
      {
        name     => $rl1_user->name,
...
# Erbo
  ...
  for my $rl1_erbo (@rl1_erboResults) {
    ...
    # Add category
    my $rl2_erbo
      = $rebus2->resultset('Category')->create({category => $rl1_erbo->erbo, rank => $rank++, source_id => 1});
    ...
    if (exists($list_links->{$rl1_sequence->list_id})) {
      ...
      if (defined($rl1_material)) {
        ...
        # Handle Everything Else
          ...
          my ($owner, $owner_uuid);
          if (defined($rl1_material->note) && $rl1_material->note =~ m/chapter|pp|page/i) {
            my @set = ('0' .. '9', 'A' .. 'F');
            my $str = join '' => map $set[rand @set], 1 .. 8;
            $csl->{title} = "Section from " . $csl->{title} . "|" . $str . "|";
            $csl->{type}  = 'chapter';
            $owner        = $config->{'code'};
            $owner_uuid   = '1-';
          }
...
# 'Permission' handled in User import above
  ...
  my $rl2_editorID
    = $rebus2->resultset('ListRole')->search({name => 'librarian'}, {rows => 1})->single->get_column('id');
