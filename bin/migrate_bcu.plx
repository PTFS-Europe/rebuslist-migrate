...
for my $rl1_list ($rl1_listResults->all) {
  ...
  my $rl2_list;
  my $rl2listResults
    = $rebus2->resultset('List')->search({course_identifier => decode_entities($rl1_list->course_identifier)});
  if ($rl2listResults->count == 0) {
    ...
    if (exists $courses{decode_entities($rl1_list->course_identifier)}) {
      $rl2_list = $dupesResult->create_rightmost_child(
        {
          name => "[" . decode_entities($rl1_list->course_identifier) . "] " . decode_entities($rl1_list->list_name),
          no_students              => $rl1_list->no_students,
          ratio_books              => $rl1_list->ratio_books,
          ratio_students           => $rl1_list->ratio_students,
          updated                  => $dt,
