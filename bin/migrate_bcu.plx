...
my $rootResult =
  ...
  ->search( {}, { order_by => { '-asc' => [qw/root_id/] }, rows => '1' } )
  ->single;
my $rootID;
...
$dupesResult->update(
    { 'source_uuid' => $config->{'code'} . "-" . $dupesResult->id } );
...
$unmappedResult->update(
    { 'source_uuid' => $config->{'code'} . "-" . $unmappedResult->id } );
...
my $rl1_listResults = $rebus1->resultset('List')
  ->search( undef, { order_by => { '-desc' => [qw/published_yn/] } } );
...
for my $rl1_user (@rl1_userResults) {
    ...
    my $email =
      defined( $rl1_user->email_address )
    ...
    my $system_role =
      ...
      : 'public';
    my $rl2_user;
    $rl2_user = $rebus2->resultset('User')
      ->find( { login => $email }, { key => 'login' } );
    $rl2_user =
      $rebus2->resultset('User')
      ->find( { login => $login }, { key => 'login' } )
    ...
    my $hashedPassword =
      defined( $rl1_user->password )
      && $rl1_user->password ne ""
      ? $rl1_user->password
      : 'acd2dd34b761c2f1ecc66982c874b2b3';
    my $ppr = Authen::Passphrase::SaltedDigest->new(
        algorithm => "MD5",
        hash_hex  => $hashedPassword
    );
...
for my $rl1_erbo (@rl1_erboResults) {
    ...
    # Add category
    my $rl2_erbo =
      $rebus2->resultset('Category')
      ->create(
        { category => $rl1_erbo->erbo, rank => $rank++, source_id => 1 } );
    $rl2_erbo->update(
        ...
        if ( defined($rl1_material) ) {
            ...
            # Handle Everything Else
                ...
                for my $rl1_tagResult ( $rl1_tagResults->all ) {
                    ...
                    # Get tag
                    my $rl1_tag = $rebus1->resultset('Tag')
                          ...
                          $rebus2->resultset('MaterialTag')->find_or_create(
                            {
                                ...
                                tag_id      => $rl2_tag->id,
                                list_id =>
                                  $list_links->{ $rl1_sequence->list_id }
                            },
                            { key => 'primary' }
                          );
                    }
...
my $permission_progress = Term::ProgressBar->new(
...
for my $rl1_uoup (@rl1_user_org_unit_permissionResults) {
        ...
        && exists( $user_links->{ $rl1_uoup->user_id } ) )
    {
        ...
        my $listResult = $rebus2->resultset('List')
          ->find( $unit_links->{ $rl1_uoup->org_unit_id } );
...
for my $rl1_ulp (@rl1_user_list_permissionResults) {
        ...
        && exists( $user_links->{ $rl1_ulp->user_id } ) )
    {
        ...
        my $listResult = $rebus2->resultset('List')
          ->find( $list_links->{ $rl1_ulp->list_id } );
        for my $descendantResult ( $listResult->descendants->all ) {
            $descendantResult->find_or_create_related(
                'list_user_roles',
                {
