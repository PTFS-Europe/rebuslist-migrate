...
my $dt    = DateTime->now;
my $start = DateTime->new(year => 2016, month => 9, day => 1, hour => 1, minute => 1, second => 1);
my $end   = DateTime->new(year => 2016, month => 8, day => 31, hour => 23, minute => 59, second => 59);
...
my $rl2_editorID = $rebus2->resultset('ListRole')->search({name => 'editor'}, {rows => 1})->single->get_column('id');
...
sub mapCSL {
  ...
  if (defined($result->title) && $result->title ne '' && !($result->title =~ /^\s*$/)) {
    $csl->{'title'} = $result->title;
  }
  if ( defined($result->authors) && $result->authors ne '' && !($result->authors =~ /^\s*$/)
  ...
  if (defined($result->edition) && $result->edition ne '' && !($result->edition =~ /^\s*$/)) {
    $csl->{'edition'} = $result->edition;
  }
  if (defined($result->volume) && $result->volume ne '' && !($result->volume =~ /^\s*$/)) {
    $csl->{'volume'} = $result->volume;
  }
  if (defined($result->issue) && $result->issue ne '' && !($result->issue =~ /^\s*$/)) {
    $csl->{'issue'} = $result->issue;
  }
  if (defined($result->publisher) && $result->publisher ne '' && !($result->publisher =~ /^\s*$/)) {
    $csl->{'publisher'} = $result->publisher;
  }
  if (defined($result->publication_date) && $result->publication_date ne '' && !($result->publication_date =~ /^\s*$/))
  {
  ...
  if (defined($result->note) && $result->note ne '' && !($result->note =~ /^\s*$/)) {
    $csl->{'note'} = $result->note;
  }
  if (defined($result->url) && $result->url ne '' && !($result->url =~ /^\s*$/)) {
    $csl->{'URL'} = $result->url;
  }

  my $epage;
  if (defined($result->epage) && $result->epage ne '' && !($result->epage =~ /^\s*$/)) {
    $epage = $result->epage;
    $epage =~ s/pp\.//g;
    $epage =~ s/\D+//g;
  ...
  my $spage;
  if (defined($result->spage) && $result->spage ne '' && !($result->spage =~ /^\s*$/)) {
    $spage = $result->spage;
    $spage =~ s/pp\.//g;
    $spage =~ s/\D+//g;
  ...
  my $secondary_title;
  if (defined($result->secondary_title) && $result->secondary_title ne '' && !($result->secondary_title =~ /^\s*$/)) {
  ...
  if ($result->material_type_id == 1) {
    ...
    if (defined($result->print_control_no) || defined($result->elec_control_no)) {
    ...
    $csl->{'number-of-pages'} = $epage - $spage if (defined($epage) && defined($spage));
    if (defined($result->print_control_no) || defined($result->elec_control_no)) {
      $csl->{'ISBN'} = defined($result->print_control_no) ? $result->print_control_no : $result->elec_control_no;
    }
  }
